...
class Rsa
{
    ...
    {
        $priKey = self::privateKey($private_key);
     ...
     */
    public static function rsaVerify($data, $sign, $public_key)
    {
        $pubKey = self::publicKey($public_key);
     ...
     */
    public function verifyBase64($data, $sign, $public_key)
    {
        $pubKey = self::publicKey($public_key);
        $res    = openssl_pkey_get_public($pubKey);
